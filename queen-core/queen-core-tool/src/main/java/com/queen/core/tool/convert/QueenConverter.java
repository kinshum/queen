...
import com.queen.core.tool.support.Try;
import com.queen.core.tool.utils.ClassUtil;
import com.queen.core.tool.utils.ConvertUtil;
...
@Slf4j
@AllArgsConstructor
public class QueenConverter implements Converter {
	 ...
	 * @param fieldName 目标的field名，原为 set 方法名，queenBeanCopier 里做了更改
	 * @return {Object}
	 */
	@Override
	@Nullable
	public Object convert(Object value, Class target, final Object fieldName) {
		...
		try {
			TypeDescriptor targetDescriptor = QueenConverter.getTypeDescriptor(targetClazz, (String) fieldName);
			return ConvertUtil.convert(value, targetDescriptor);
		} catch (Throwable e) {
			log.warn("QueenConverter error", e);
			return null;
		}
	}
